...
if( ! function_exists( 'digeco_related_post' )){
	
	function digeco_related_post(){
			
		$post_id = get_the_id();	
		$number_of_avail_post = '';
		$current_post = array( $post_id );	
		$title_length = DigecoTheme::$options['show_related_post_title_limit'] ? DigecoTheme::$options['show_related_post_title_limit'] : '';
		$related_post_number = DigecoTheme::$options['show_related_post_number'];
		...
		<div class="owl-wrap rt-woo-nav rt-related-post related post <?php echo esc_attr( $wrapper_class );?>">
			<div class="title-section">
				<h2 class="owl-custom-nav-title"><?php esc_html_e ( 'Related Post', 'digeco' ); ?></h2>
				<?php if ( $count_post > 3 ){ ?>
			...
			<div class="owl-theme owl-carousel rt-owl-carousel" data-carousel-options="<?php echo esc_attr( $owl_data );?>">
				...
				?>
					<div class="position-relative">
						<?php if ( has_post_thumbnail() || DigecoTheme::$options['display_no_prev_img_related_post'] == '1'  ) { ?>
						<a href="<?php the_permalink(); ?>">
							...
							<?php
								...
								if ( has_post_thumbnail() ){
									...
									if ( DigecoTheme::$options['display_no_prev_img_related_post'] == '1' ) {
									$thumbnail = '<img class="" src="'.DIGECO_IMG_URL.'noimage_450X330.jpg" alt="'. the_title_attribute(array('echo'=> false)) .'">';
									}
								}
						...
						<div class="rt-related-post-info">
							<div class="post-date">
								<ul>
									<?php if ( DigecoTheme::$options['show_related_post_date'] ) { ?>
									<li><?php echo get_the_date(); ?></li>
									<?php } if ( DigecoTheme::$options['show_related_post_cat'] ) { ?>
									<li><?php echo esc_html( digeco_get_primary_category()[0]->name ); ?></li>
									<?php } ?>
								</ul>
							</div>
							<h3 class="post-title">
